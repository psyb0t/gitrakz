...
There is no `go get github.com/psyb0t/gitrakz` step. `internal/`, `cmd/main.go`, and `pkg/runner/` are not designed to be imported into an existing module — they're the skeleton of YOUR module. You clone the repo, rewrite the module path, and build on top.
...
Framework code imports as `github.com/psyb0t/gitrakz/...` before you run `make own`; afterwards every import is rewritten to your module path, e.g. `github.com/yourname/yourproject/internal/app`.
